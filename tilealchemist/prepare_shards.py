...
tilealchemist/sources/ for how the archive URL itself gets resolved.
    ...
    tilealchemist-prepare-shards --worker-count 256 --max-zoom 14 \
        --out-dir manifests/
"""
import argparse
...
# WORKER_COUNT (256) so this one-time walk doesn't add extra load on top of
# the much larger per-shard fetch fan-out that follows, on OpenFreeMap's
# single free community-run server.
...
def _wait_before_retry(attempt, response, range_header, reason=""):
    ...
    print(f"prepare: got {response.status_code} for range {range_header}{reason} "
        ...
        if response.status_code != 206:
            if attempt == MAX_RANGE_ATTEMPTS:
                raise RuntimeError(
                    ...
                    f"header and would send the entire archive instead of just this range"
                )
...
def fetch_directory_node(session, url, header, node_offset, node_length, tile_id_limit):
...
def walk_directory_tree(session, url, header, tile_id_limit):
        ...
        while frontier or pending:
            while frontier and len(pending) < WALK_WORKER_COUNT:
                ...
                pending.add(executor.submit(
                    fetch_directory_node, session, url, header, node_offset, node_length, tile_id_limit))
            ...
            if throttle.due():
                print(f"prepare: walked {dirs_walked} directories, {len(entries)} entries so far "
                      f"(in flight: {len(pending)})", file=sys.stderr)

    return entries


def collect_entries(session, url, max_zoom):
    ...
    entries = walk_directory_tree(session, url, header, tile_id_limit)
...
def main():
    ...
    parser.add_argument("--worker-count", type=int, default=256)
    ...
    print(f"prepare: source={url} (build {timestamp})", file=sys.stderr)

    # one-time directory walk
    session = make_session(WALK_WORKER_COUNT)
    header, entries = collect_entries(session, url, args.max_zoom)
    print(f"prepare: directory walk found {len(entries)} distinct tile entries "
    ...
    print(f"prepare: {len(gaps)} gap ranges covering {gap_tile_count} tiles with no archive "
    ...
    blocks = [real_blocks[worker_index] + gap_blocks[worker_index] for worker_index in range(args.worker_count)]
    ...
    print(f"prepare: wrote {len(blocks)} manifests to {args.out_dir} "
