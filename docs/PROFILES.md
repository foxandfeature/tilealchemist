...
- **`vector_layers_json()`** returns the `vector_layers` array embedded in
  ...
  `[{"id": "land", "fields": {}}]`.
- **`transform_tile_bytes(data)`** is the actual per-tile work: one gzipped
  source MVT tile's bytes in, gzipped output MVT bytes out, or `None` to
  skip (write nothing for this tile). Not named after any specific
  operation like "invert" or "crop": a profile can do whatever it wants
  here.
- **`gap_tile_bytes(zoom, tile_column, tile_row)`** is called once per *gap*
  ...
  single large gap (e.g. an ice sheet interior).

`build_shard.py` (the generic worker script) only ever calls these three
methods, nothing profile-specific.

## Shared helpers a profile can use

Neither is part of the `Profile` contract:

- **`tilealchemist/mvt.py`** (`decode_tile()`/`encode_tile()`) wraps the
  gzip+MVT codec every profile ends up needing, since both the source
  archive's tiles and this pipeline's own output rows are gzipped MVT.
  Generic, unrelated to any profile's domain.
- **`tilealchemist/water.py`** (`union_polygons()`) is geometry math for
  ...
  POIs, or anything else has no reason to touch it.
...
class OpenMapTilesSchema(TileSchema):
    ...
    def surface_water(self, decoded_tile):
        ...
        if not water:
            return None
        polygons = [shape(f["geometry"]) for f in water["features"]
    ...
    def waterway_lines(self, decoded_tile):
        ...
        if not waterway:
            return None
        features = [{"geometry": shape(f["geometry"]), "properties": f["properties"]}
...
Once your package is installed alongside `tilealchemist`,
